...
def preparar_dados_para_ia(dados, acao):
    ...
    try:
        data = {
            ...
            "ev_ebit": valuation_indicators.get('enterprise_value_by_ebit', None).value if valuation_indicators.get('enterprise_value_by_ebit') else None,
            ...
            "ebit_12meses": income_statement_data.get('twelve_months', {}).get('ebit', None).value if income_statement_data.get('twelve_months', {}).get('ebit') else None,
...
def analisar_acao(acao, resultados, acao_status):
    ...
    resultados[acao] = {"analise": analise_ia, "classificacao": classificacao, "dados": df_dados} # Armazena a classificação e os dados
